...
interface StepperProps extends HTMLAttributes<HTMLDivElement> {
  ...
  allowsStep?: (fromStep: number, toStep: number) => boolean;
...
export default function Stepper({
  ...
  const updateStep = (newStep: number) => {
    ...
    if (allowsStep && !allowsStep(fromStep, newStep)) {
      return; // navigation blocked (e.g. invalid form step)
    }
  ...
  return (
    ...
    >
      <div
        className={`px-4 mx-auto w-full rounded-4xl shadow-xl border border-border bg-background ${stepCircleContainerClassName}`}
      >
          ...
          <div className={`px-3 pb-8 ${footerClassName}`}>
            <div className={`mt-5 flex ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`}>
              {currentStep !== 1 && (
                <button
                  onClick={handleBack}
                  className={`duration-350 py-1.5 px-3.5 transition cursor-pointer border-border border rounded-full ${
                    currentStep === 1
              ...
              <button
                ...
                className="duration-350 flex items-center justify-center rounded-full bg-primary py-1.5 px-3.5 font-medium tracking-tight text-primary-foreground transition hover:bg-primary-hover cursor-pointer"
                {...nextButtonProps}
              >
